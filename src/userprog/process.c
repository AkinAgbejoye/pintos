...
/* exit the process to the program name is null */
...
start_process (void *file_name_)
{
  ...
  //tokenisation and getting parameters
  argc   = 0;
...
for(token =  strtok_r(file_name, " ", &save_ptr);
  ...
  // push command and its parameters to stack
  if(success){
  if_.esp -= file_name_len + 1;
  start = if_.esp;
  memcpy(if_.esp,file_name, file_name_len +1);
  //round to a multiple of 4
  if_.esp  -= 4 -((file_name_len + 1) % 40);
  
 *(int *) if_.esp = 0;
  if_.esp -= 4;
  *(int *) if_.esp = 0;

 //push elememnt right to the left
 for( i = argc -1; i >= 0; i--) {
